<?php

declare(strict_types=1);

namespace App\Services\Calendar;

use App\ValueObjects\Calendar\PublicHoliday;
use App\ValueObjects\Calendar\PublicHolidayCalendar;
use InvalidArgumentException;

/**
 * Provides Egypt's official public-holiday calendar for a given Gregorian year.
 *
 * Fixed national holidays are sourced from Egyptian law and presidential decrees.
 * Islamic-based holidays use astronomical Hijri-to-Gregorian conversions and are
 * pre-populated for 2025–2027; exact dates may shift by ±1 day pending
 * moon-sighting declarations.
 */
final class EgyptHolidayCalendarService
{
    public const int MIN_YEAR = 2025;
    public const int MAX_YEAR = 2027;

    public function forYear(int $year): PublicHolidayCalendar
    {
        if ($year < self::MIN_YEAR || $year > self::MAX_YEAR) {
            throw new InvalidArgumentException(
                "Egypt holiday calendar is pre-populated for {$year}. Supported range: "
                . self::MIN_YEAR . '–' . self::MAX_YEAR . '.',
            );
        }

        return new PublicHolidayCalendar('EG', $year, $this->holidays($year));
    }

    /** @return PublicHoliday[] */
    private function holidays(int $year): array
    {
        return [
            ...$this->fixedHolidays($year),
            ...$this->islamicHolidays($year),
        ];
    }

    /** @return PublicHoliday[] Fixed Gregorian-date holidays. */
    private function fixedHolidays(int $year): array
    {
        return [
            new PublicHoliday(
                date: "{$year}-01-07",
                name: 'Coptic Christmas',
                nameAr: 'عيد الميلاد المجيد',
                type: 'religious_coptic',
                daysOff: 1,
            ),
            new PublicHoliday(
                date: "{$year}-01-25",
                name: 'January 25th Revolution Day / Police Day',
                nameAr: 'ثورة يناير / عيد الشرطة',
                type: 'national',
                daysOff: 1,
            ),
            new PublicHoliday(
                date: "{$year}-04-25",
                name: 'Sinai Liberation Day',
                nameAr: 'تحرير سيناء',
                type: 'national',
                daysOff: 1,
            ),
            new PublicHoliday(
                date: "{$year}-05-01",
                name: "Labour Day",
                nameAr: 'عيد العمال',
                type: 'international',
                daysOff: 1,
            ),
            new PublicHoliday(
                date: "{$year}-06-30",
                name: 'June 30th Revolution Day',
                nameAr: 'ثورة 30 يونيو',
                type: 'national',
                daysOff: 1,
            ),
            new PublicHoliday(
                date: "{$year}-07-23",
                name: 'July 23rd Revolution Day',
                nameAr: 'ثورة 23 يوليو',
                type: 'national',
                daysOff: 1,
            ),
            new PublicHoliday(
                date: "{$year}-10-06",
                name: 'Armed Forces Day',
                nameAr: 'عيد القوات المسلحة',
                type: 'national',
                daysOff: 1,
            ),
        ];
    }

    /**
     * Islamic-based holidays (shift ~11 days earlier each Gregorian year).
     * Dates are astronomical estimates for 2025–2027.
     *
     * @return PublicHoliday[]
     */
    private function islamicHolidays(int $year): array
    {
        $data = [
            2025 => [
                ['2025-03-30', 'Eid al-Fitr',        'عيد الفطر المبارك',   3, true],
                ['2025-06-06', 'Eid al-Adha',        'عيد الأضحى المبارك',  4, true],
                ['2025-06-26', 'Islamic New Year',   'رأس السنة الهجرية',   1, true],
                ['2025-09-04', "Prophet's Birthday", 'المولد النبوي الشريف', 1, true],
            ],
            2026 => [
                ['2026-03-19', 'Eid al-Fitr',        'عيد الفطر المبارك',   3, true],
                ['2026-05-26', 'Eid al-Adha',        'عيد الأضحى المبارك',  4, true],
                ['2026-06-16', 'Islamic New Year',   'رأس السنة الهجرية',   1, true],
                ['2026-08-26', "Prophet's Birthday", 'المولد النبوي الشريف', 1, true],
            ],
            2027 => [
                ['2027-03-09', 'Eid al-Fitr',        'عيد الفطر المبارك',   3, true],
                ['2027-05-16', 'Eid al-Adha',        'عيد الأضحى المبارك',  4, true],
                ['2027-06-05', 'Islamic New Year',   'رأس السنة الهجرية',   1, true],
                ['2027-08-15', "Prophet's Birthday", 'المولد النبوي الشريف', 1, true],
            ],
        ];

        return array_map(
            fn (array $h) => new PublicHoliday(
                date: $h[0],
                name: $h[1],
                nameAr: $h[2],
                type: 'religious_islamic',
                daysOff: $h[3],
                islamicBased: $h[4],
            ),
            $data[$year],
        );
    }
}
